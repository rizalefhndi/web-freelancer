                ...
                <div class="flex overflow-x-scroll hide-scroll-bar dragscroll">
                     <div class="flex mt-2 flex-nowrap">
                        @forelse ($thumbnail as $item)
                            ...
                            <img :class="{ 'border-4 border-collabio-button': active === {{ $item->id }} }" @click="changeThumbnail('{{ url(Storage::url($item->thumbnail)) }}', {{ $item->id }})" src="{{ url(Storage::url($item->thumbnail)) }}" alt="thumbnail service" class="inline-block mr-2 rounded-lg cursor-pointer h-20 w-36 object-cover">
                            ...
                            <div class="flex items-center col-span-12 p-2 lg:col-span-6">
                                <div class="flex items-center space-x-4">
                                    @if ($service->user->detail_user->photo != NULL)
                                        <img class="w-20 h-2- object-cover rounded-full" src="{{ url(Storage::url($service->user->detail_user->photo)) }}" alt="photo profile" loading="lazy">
                                    @else
        ...
        <aside class="p-4 lg:col-span-4 md:col-span-12 md:pt-0">
            <div class="mb-4 border rounded-lg border-collabio-testimonial-border">
                ...
                <div class="flex items-start px-4 pt-6">
                    ...
                    <div class="w-full">
                        <div class="flex items-center justify-between">
                            <h2 class="my-1 text-xl font-medium text-collabio-bg">{{ $service->user->name ?? '' }}</h2>
                        </div>
                ...
                <div class="px-4 pb-4 booking">
                    ...
                    @guest
                        <a onclick="toggleModal('loginModal')" class="block px-12 py-4 my-2 text-lg font-semibold text-center text-white bg-collabio-button rounded-xl">
                            Booking Now
                        </a>
                    @endguest
